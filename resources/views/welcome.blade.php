...
<body class="antialiased">
    <div
        ...
        <div class="max-w-7xl mx-auto p-6 lg:p-8">
            ...
            <select class="form-select mt-2" id="ciudad" disabled>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const countryId = 3686110; // Colombia (Geonames)

    // 1️⃣ Cargar departamentos
    fetch(`/api/countries/${countryId}/departments`)
        .then(res => res.json())
        .then(data => {
            data.forEach(dep => {
                const option = document.createElement('option');
                option.value = dep.geonameId; // geoname_id
                option.textContent = dep.name;
                departamentoSelect.appendChild(option);
            });
        });

    // 2️⃣ Cuando cambie el departamento → cargar ciudades
    departamentoSelect.addEventListener('change', () => {
        ...
        fetch(`/api/departments/${departmentId}/cities`)
            .then(res => res.json())
            .then(data => {
                ciudadSelect.innerHTML = '<option value="">Seleccione una ciudad</option>';
...
<script>
    ...
    function loadDepartamentos() {
        ...
        $.ajax({
            url: `/api/countries/${colombiaGeonameId}/departments`,
            method: 'GET',
